...
SELECT EMP_NAME, ABS(CEIL(HIRE_DATE - SYSDATE)) 근무일수1, CEIL(SYSDATE - HIRE_DATE) 근무일수2
FROM EMPLOYEE;
...
SELECT *
FROM EMPLOYEE
WHERE 25-SUBSTR(HIRE_DATE,0,2)>=20 OR SUBSTR(HIRE_DATE,0,2)> 70; 
...
SELECT EMP_NAME, DEPT_CODE, SUBSTR(EMP_NO,1,2)||'년',SUBSTR(EMP_NO,3,2)||'월',SUBSTR(EMP_NO,5,2)||'일',100 - SUBSTR(EMP_NO,1,2) + SUBSTR(SYSDATE,1,2) 나이
FROM EMPLOYEE;
...
--=====================================================================================
/*
    *DECODE(비교하고자하는 대상(컬럼)|산술연산|함수식),비교값1,비교값2,비교값3,...)
   switch(비교대상){
      case 비교값1:
        결과값1;
        break;
