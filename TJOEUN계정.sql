...
FROM EMPLOYEE;
-->산술연산 과정 중 NULL값이 존재할 경우 산술연한 결과값도 무조건 NULL값으로 나옴
...
/*
    <연결 연산자: ||>
    여러 컬럼값들을 마치 하나의 커럼인것처럼 연결하거나, 컬럼값과 리터럴을 연결할 수 있음
*/
...
SELECT EMP_ED,EMP_NAME||SALARY 종합정보
FROM EMPLOYEE;
...
/*
    ...
    OR(~이거나, 또는)
*/
--EMPLOYEE테이블에서 부서코드가 'D9'이면서 급여사 500만원 이상인 사원들의 사원명,부서코드,급여 조회
SELECT EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
...
--EMPLOYER테이블에서 사원들 이름에 '하'가 포함되어 있는 사원들의 사원명,주민번호,전화번호 조회
SELECT EMP_NAME, EMP_NO, PHONE
FROM EMPLOYEE
WHERE EMP_NAME LIKE '%하%';

--EMPLOYEE테이블에서 사원들 이름에 '하'가 중간에 들어있는 사원들의 사원명,주민번호,전화번호 조회
SELECT EMP_NAME, EMP_NO, PHONE
FROM EMPLOYEE
...
--EMPLOYEE테이블에서 이메일에_앞에 글자가 3글자인 사원의 사원명,이메일 조회
...
--EMPLOYEE테이블에서 직급코드가 'J7'이거나 'J2'인 직원들 중 급여가 200만원 이상인 사원들의 모든 커럼 조회
SELECT *
FROM EMPLOYEE
...
--EMPLOYEE테이블의 모든 컬럼 조회. 정렬기준 보너스의 내림차순 정렬. 보너스가 같으면 급여의 오름차순 정렬로 조회
SELECT*
FROM EMPLOYEE
...
SELECT *
FROM EMPLOYEE
WHERE EMAIL LIKE '____$_%'ESCAPE '$' AND (DEPT_CODE = 'D9' OR DEPT_CODE = 'D6') AND (HIRE_DATE BETWEEN '90/01/01' AND '00/12/01') AND
SALARY >= 2700000;
