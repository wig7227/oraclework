...
AS SELECT DEPARTMENT_NAME 학과, COUNT(DEPARTMENT_NO) "학생 수"
FROM TB_DEPARTMENT
...
--15.
CREATE OR REPLACE VIEW VW_인기과목
AS SELECT CLASS_NO 과목번호,COUNT(*)"누적수강생수(명)"
FROM TB_GRADE
WHERE SUBSTR(TERM_NO,1,4) IN ('2008','2007','2006')
AND ROWNUM <= 3
GROUP BY CLASS_NO
ORDER BY 2 DESC ;
