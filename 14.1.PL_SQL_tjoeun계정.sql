...
---------------------------------실습 문제--------------------------------------
/*
    레퍼런스 타입변수를 EID, ENAME, JCODE, SAL, DTITLE을 선언하고
    각 자료형을 테이블을 참조하여 넣기
    
    사용자가 입력한 사번의 사원으 변수에 담겨있는 값 출력
*/
...
DECLARE
    EID EMPLOYEE.EMP_ID% TYPE;
    ENAME EMPLOYEE.EMP_NAME%TYPE;
    DTILTE DEPARTMENT.DEPT_TITLE%TYPE;
    NCODE LOCATION.NATIONAL_CODE%TYPE;
    TEAM VARCHAR2(20);
BEGIN
    SELECT EMP_ID, EMP_NAME, DEPT_TITLE, NATIONAL_CODE
    INTO EID, ENAME, DTILTE, NCODE
    FROM EMPLOYEE
    JOIN DEPARTMENT ON (DEPT_ID = DEPT_CODE)
...
--    WHERE NATIONAL_CODE = &근무국가코드;
    ...
    DBMS_OUTPUT.PUT_LINE('부서명:' || DTILTE);
...
-- 사용자로부터 입력받은 사번의 사원을 부서코드를 이용하여 부서이름으로 변경하기
DECLARE
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('연봉: '||SAL*(BONUS+1)*12);
    ELSE
    DBMS_OUTPUT.PUT_LINE('연봉: '||SAL*12);
    END IF;
EXCEPTION
    WHEN TOO_MANY_ROWS THEN DBMS_OUTPUT.PUT_LINE('한 행만 조회가능');
    WHEN NO_DATA_FOUND THEN DBMS_OUTPUT.PUT_LINE('조회 결과가 없습니다');
     ...
     LOOP
         IF MOD(DAN,2) = 0
            THEN
                ...
                LOOP
                TOTAL := DAN * X;
                DBMS_OUTPUT.PUT_LINE(DAN||'단'||TOTAL);
                END LOOP;
